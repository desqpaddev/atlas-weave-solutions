...
export default function PaymentsPage() {
  ...
  // For customers, get their booking IDs first
  const { data: myBookingIds = [] } = useQuery({
    queryKey: ["my-booking-ids", customerRecord?.id],
    queryFn: async () => {
      const { data } = await supabase.from("bookings").select("id").eq("customer_id", customerRecord!.id);
      return data?.map(b => b.id) || [];
    },
    enabled: isCustomer && !!customerRecord?.id,
  });
  ...
  const { data: payments = [], isLoading } = useQuery({
    queryKey: ["payments", isCustomer ? myBookingIds : "all"],
    queryFn: async () => {
      let query = supabase.from("payments").select("*, bookings(reference_number, title)").order("created_at", { ascending: false });
      if (isCustomer && myBookingIds.length > 0) {
        query = query.in("booking_id", myBookingIds);
      }
      const { data, error } = await query;
      if (error) throw error;
      return data;
    },
    enabled: isCustomer ? (customerRecord !== undefined) : true,
  });
  ...
  // For customers with no bookings, show empty state
  const showEmpty = isCustomer && (customerRecord === null || myBookingIds.length === 0) && payments.length === 0 && !isLoading;

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground">{isCustomer ? "My Payments" : "Payments"}</h1>
          <p className="text-muted-foreground text-sm">{isCustomer ? "View your payment history." : "Track payment transactions."}</p>
        </div>
        {!isCustomer && (
      ...
      {showEmpty ? (
        <div className="text-center py-16">
          ...
          <h2 className="text-xl font-display font-semibold text-foreground mb-2">No payments yet</h2>
          <p className="text-muted-foreground">Your payment history will appear here after you make a booking.</p>
        </div>
      ) : (
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <Table>
            <TableHeader><TableRow className="border-border hover:bg-transparent"><TableHead>Booking</TableHead><TableHead>Amount</TableHead><TableHead>Method</TableHead><TableHead>Status</TableHead><TableHead className="hidden sm:table-cell">Date</TableHead>{!isCustomer && <TableHead>Actions</TableHead>}</TableRow></TableHeader>
            <TableBody>
